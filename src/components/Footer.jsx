  ...
  return (
    <footer className="pt-14 pb-8 bg-white">
      ...
      >
        {/* Footer top */}
        <div
          className="grid gap-10 sm:grid-cols-4 lg:grid-cols-
        [1fr_0.7fr_O.7fr_0.7fr]"
        >
          {/* Footer brand */}
            ...
            <div className="mt-8 space-y-4">
              {contactInfo.map((item) => (
                <a
                  ...
                  hover:text-orange-50 hover:underline transition-colors"
                >
                  {<item.icon />}
                  {item.label}
                </a>
          ...
          <motion.div variants={variants.fadeInUp}>
            ...
            <div className="flex mt-5 gap-3">
              {socialIcons.map((icon) => (
                <button
                  className="secondary-btn p-3.5 bg-white-97
                hover:bg-white-99 hover:shadow-md transition-all"
                >
                  <a href="#">{<icon.icon />}</a>
                </button>
              ))}
